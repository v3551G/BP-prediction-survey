...
"""
    Preprocessing script for PPG data downloaded from the MIMIC-III database.
...
def CreateWindows(win_len, fs, N_samp, overlap):
    '''
    Generate the windows for splitting subject record into samples
    :param win_len: window length (unit: second)
    :param fs: sampling frequency
    :param N_samp:
    :param overlap: the proportion of overlap
    :return:
    '''
    win_len = win_len * fs
...
def prepare_MIMIC_dataset(DataPath, OutputFile, NsampPerSubMax: int=None, NsampMax: int=None, win_len: int=7,
                          win_overlap: float = 0.5, savePPGData=False):
    '''
    Filter signals and then divide synchronous PPG and ABp signals into samples based on fixed window length.
    Then exclude artifacts from the dataset. Finally, save the processed data in into a .h5 file.
    :param DataPath:
    :param OutputFile:
    :param NsampPerSubMax:
    :param NsampMax:
    :param win_len:
    :param win_overlap:
    :param savePPGData:
    ...
    b, a = butter(N=4, Wn=[0.5, 8], btype='bandpass', fs=fs)
    ...
    for idx, dirs in enumerate(SubjectDirs):
        print(
            f'{datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}: Processing subject {idx + 1} of {NumSubjects} ({dirs.name}): ',
            end='')
        ...
        for file in DataFiles:
            ...
            for i in range(0, N_win):
                ...
                # DBP peaks
                ABP_sys_idx_win = ABP_sys_idx[np.logical_and(ABP_sys_idx >= idx_start, ABP_sys_idx < idx_stop)].astype(
                    int)
                ABP_dia_idx_win = ABP_dia_idx[np.logical_and(ABP_dia_idx >= idx_start, ABP_dia_idx < idx_stop)].astype(
                    int)
